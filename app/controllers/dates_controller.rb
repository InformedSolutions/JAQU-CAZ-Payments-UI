...
class DatesController < ApplicationController # rubocop:disable Metrics/ClassLength
  ...
  #
  def select_period
    @return_path = case params['second_week']
                   when 'false'
  ...
  def check_if_pay_week_starts_today(service)
    return unless service.pay_week_starts_today? && vehicle_details('confirm_weekly_charge_today') != false
  ...
  def determinate_return_path
    la_name == 'Weeklydiscountcaz' && weekly_possible ? select_period_dates_path : local_authority_charges_path
  end
  ...
  def handle_back_action(second_week_selected:, session_date:, back_button_date:)
    ...
    elsif !session_date && back_button_date
